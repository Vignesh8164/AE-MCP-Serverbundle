...
const llmProvider = (0, factory_1.createLLMProvider)();
...
app.post('/api/command', (req, res) => {
    ...
    const cmd = { id: commandId, tool, params, type: tool, status: 'pending' };
...
function finalizeCommand(id, status, result, error) {
    ...
    cmd.result = result;
    cmd.error = error;
    if (status === 'completed' && cmd.resolve)
        cmd.resolve(result);
    if (status === 'failed' && cmd.reject)
        cmd.reject(new Error(error || 'failed'));
    return true;
}
